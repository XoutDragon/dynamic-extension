...
#include "framework/structure/Checkpoint.h"
...
namespace de
{
  ...
  public:
    ExtensionStructure(size_t buffer_size, size_t scale_factor,
                       double max_delete_prop, bool skip_checkpoint = false)
          ...
          m_buffer_size(buffer_size)
    {
      ...
      load_checkpoint(m_levels);

      size_t shard_capacity = (L == LayoutPolicy::LEVELING) ? 1 : m_scale_factor;

      for (size_t i = 0; i < m_levels.size(); i++)
      {
        size_t rec_cnt = (m_levels[i]) ? m_levels[i]->get_record_count() : 0;
        size_t rec_cap = calc_level_record_capacity(i);
        size_t shard_cnt = (m_levels[i]) ? m_levels[i]->get_shard_count() : 0;
        size_t shard_cap = shard_capacity;

        m_current_state.push_back({rec_cnt, rec_cap, shard_cnt, shard_cap});
      }
    }
    ...
    ExtensionStructure<ShardType, QueryType, L> *copy()
    {
      auto new_struct = new ExtensionStructure<ShardType, QueryType, L>(
          m_buffer_size, m_scale_factor, m_max_delete_prop, true);
      for (size_t i = 0; i < m_levels.size(); i++)
      {
