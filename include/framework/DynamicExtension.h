...
namespace de
{
  ...
  public:
    ...
    DynamicExtension(size_t buffer_low_watermark, size_t buffer_high_watermark,
                     ...
                     size_t thread_cnt = 16, bool skip_checkpoint = false)
          ...
          m_core_cnt(thread_cnt), m_next_core(0), m_epoch_cnt(0)
    {
      ...
      auto vers =
          new Structure(buffer_high_watermark, m_scale_factor, m_max_delete_prop, skip_checkpoint);
     ...
     *          be retried)
     */
    int insert(const RecordType &rec, bool logging = true) { return internal_append(rec, false, logging); }
     ...
     */
    int erase(const RecordType &rec, std::byte *buffer = nullptr, bool logging = true)
    {
       ...
       */
      return internal_append(rec, true, logging);
    }
  ...
  private:
    ...
    int internal_append(const RecordType &rec, bool ts, bool logging)
    {
      ...
      return m_buffer->append(rec, ts, logging);
    }
